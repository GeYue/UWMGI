...
def build_dataloader(df, fold, data_transforms, CFG):
    ...
    train_loader = DataLoader(train_dataset, batch_size=CFG.train_bs, num_workers=42, shuffle=True, pin_memory=True, drop_last=False)
    valid_loader = DataLoader(valid_dataset, batch_size=CFG.valid_bs, num_workers=42, shuffle=False, pin_memory=True)
...
def train_one_epoch(model, train_loader, optimizer, losses_dict, CFG):
    ...
    pbar = tqdm(enumerate(train_loader), total=len(train_loader), desc=f'Train[{model.current_fold}] ')
    for _, (images, masks) in pbar:
        ...
        pbar.set_postfix(train_loss=f'{losses_all:0.4f}',
                        lr=f'{current_lr:0.5f}',
                        gpu_mem=f'{mem:0.2f} GB',
                        epoch=f"{model.current_epoch}/{CFG.epoch}",)
...
def valid_one_epoch(model, valid_loader, CFG):
    ...
    pbar = tqdm(enumerate(valid_loader), total=len(valid_loader), desc=f'Valid[{model.current_fold}] ')
    for _, (images, masks) in pbar:
        images  = images.to(CFG.device, dtype=torch.float) # [b, c, w, h]
        masks   = masks.to(CFG.device, dtype=torch.float)  # [b, c, w, h]
        batch_size = images.size(0)
        ...
        mem = torch.cuda.memory_reserved() / 1E9 if torch.cuda.is_available() else 0
        pbar.set_postfix(valid_loss=f'{losses_all:0.4f}',
    ...
    return val_dice, val_jaccard, losses_all
...
if __name__ == '__main__':
    ...
    if CFG.wandb:
        ...
        run = wandb.init(project="UMWGI", 
                        #name="York_PPPM",
                        config=class2dict(CFG), #class2dict(CFG),
                        #group="DeBERTa-V3L",
                        #job_type="train",
                        )
    ...
    if train_val_flag:
        ...
        ##### part0: data preprocess
        ###############################################################
        path_df = pd.DataFrame(glob('./ram/generation/images/*'), columns=['image_path'])
        ...
        for fold in range(CFG.n_fold):
            ...
            for epoch in range(1, CFG.epoch+1):
                ...
                ##### >>>>>>> step3: train & val <<<<<<
                ###############################################################
                train_loss = train_one_epoch(model, train_loader, optimizer, losses_dict, CFG)
                lr_scheduler.step()
                val_dice, val_jaccard, val_loss = valid_one_epoch(model, valid_loader, CFG)
