...
test_df = add_image_paths(test_df, channels=CFG['2.5D_layers'], stride=CFG['layer_stride'])
test_df.head()
...
def Inference(model, dataloader, threshold):
    ...
    for step, (images, ids, heights, widths) in pbar:        
        ...
        masks = (masks.permute((0, 2, 3, 1)) > threshold).to(torch.uint8).cpu().detach().numpy()  # shape: (n, h, w, c)
...
pred_df = Inference(model, test_loader, threshold=0.45)
