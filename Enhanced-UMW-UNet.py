...
CFG = {
    "seed": 1979,
    "arch": "UnetPlusPlus", ## FPN / DeepLabV3 / DeepLabV3Plus / Unet / UnetPlusPlus
    "encoder_name": 'efficientnet-b2', ## efficientnet-b7 / timm-efficientnet-b8 / timm-efficientnet-l2 / timm-resnest200e 
...
if CFG['wandb']:
    ...
    #run = wandb.init(id='phyp0ycg', resume="must")
...
def prepare_loaders(fold):
    ...
    train_loader = DataLoader(train_dataset, batch_size=CFG['train_bs'], 
                              num_workers=CFG['num_workers'], shuffle=True, pin_memory=False, drop_last=False)
...
def add_image_paths(df, channels=3, stride=2):
    ...
    for i in range(channels):
        if i == 0:
            df[f"image_path_{i:02}"] = df.groupby(["case", "day"])["image_path"].shift(2).fillna("")
        elif i == 1:
            df[f"image_path_{i:02}"] = df.groupby(["case", "day"])["image_path"].shift(1).fillna("")
        elif i == 2:
            df[f"image_path_{i:02}"] = df.groupby(["case", "day"])["image_path"].shift(0).fillna("")
        elif i == 3:
            df[f"image_path_{i:02}"] = df.groupby(["case", "day"])["image_path"].shift(-1).fillna("")
        elif i == 4:
            df[f"image_path_{i:02}"] = df.groupby(["case", "day"])["image_path"].shift(-2).fillna("")
    ...
    for i in range(len(df)):
        ...
        strValue = f"case{df.iloc[i]['case']}_day{df.iloc[i]['day']}/scans/"
        for j in range(len(df.iloc[i]['image_paths'])):
            img = df.iloc[i]['image_paths'][j]
            if strValue not in img:
                df.iloc[i]['image_paths'][j] = path
    """
    return df
...
df = add_image_paths(df, channels=CFG['2.5D_layers'], stride=CFG['layer_stride'])
